           ...
           Statistics * stats)
{
  ...
  std::cout << "edges:" << optimizer.edges().size() << "\n";
  std::cout << "verte:" << optimizer.vertices().size() << "\n";
  g2o::HyperGraph::Edge e;
  typedef std::set<g2o::HyperGraph::Edge*>::const_iterator EdgeIterator;
//  for(EdgeIterator itr=optimizer.edges().begin(); itr != optimizer.edges().end(); itr++)
//    std::cout << "edge " <<  (**itr).id() << " vertex pointer " << (**itr).vertex(0) << "\n";
  optimizer.save("g2o_output.g2o");
